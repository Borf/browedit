...
void MapRenderer::renderGnd(blib::Renderer* renderer, const Map* map)
{
	if(gndVbo == NULL)
	{
		...
		for(auto it : verts)
		{
			gndVertIndices[it.first] = std::pair<int,int>(allVerts.size(), it.second.size());
			allVerts.insert(allVerts.end(), it.second.begin(), it.second.end());
		}
		gndVbo = resourceManager->getResource<blib::VBO>();
		gndVbo->setVertexFormat<blib::VertexP3T2>();
		renderer->setVbo(gndVbo, allVerts);

	}


	//render using VBOs for nice speeds in debugbuilds
	gndRenderState.activeShader->setUniform("modelViewMatrix", cameraMatrix);
	gndRenderState.activeVbo = gndVbo;

	for(auto it : gndVertIndices)
	{
